...
# --- Capture resolution ---
# D435/D435i/D455 support 1280x720 @ 30 fps on both streams over USB3.
# If you want to go bigger, try 1920x1080 color (depth max is 1280x720).
# If your USB struggles, drop depth to (848, 480) — alignment will upscale.
COLOR_W, COLOR_H = 1280, 720
DEPTH_W, DEPTH_H = 1280, 720
FPS = 30

# --- UI scaling (relative to old 640x480 layout) ---
UI_SCALE = COLOR_W / 640.0   # 2.0 at 1280x720, 3.0 at 1920x1080

# Tunable thresholds
_p = int(round(7 * UI_SCALE))
DEPTH_PATCH_SIZE     = _p if _p % 2 == 1 else _p + 1   # keep odd
MAX_FIT_ERR_M        = 0.010                            # 10 mm Kabsch residual
MAX_REPROJ_ERR_PX    = 2.0 * UI_SCALE                   # scales with image
...
# Resizable window — drag the corner to whatever size you want
...
# ============================================================
# UI sizes (scaled once, used everywhere)
# ============================================================

S_BIG    = 0.55 * UI_SCALE      # status (cam pos)
S_MED    = 0.50 * UI_SCALE      # kabsch fit, click labels
S_SMALL  = 0.42 * UI_SCALE      # diagnostics, per-marker
S_FOOT   = 0.45 * UI_SCALE      # footer
...
print("L=test  R=wis  SPACE=lock/unlock  C=clear hist  "
      "S=save  D=load  ESC=stop")

# Auto-load existing calibration if present
...
try:
    while True:
        ...
        if ids is not None:
            ...
            # Voeg huidige frame centers toe aan history
            ...
            # Paarsgewijze afstanden (huidige frame, depth-based)
        ...
        # KABSCH op temporeel gemiddelde (mediaan) van history
        ...
        if len(ids_for_kabsch) >= 3:
            P = np.array(
                [WORLD_MARKERS[m] for m in ids_for_kabsch],
                dtype=np.float64
            )
            Q = np.array([
            ...
            if T_cam_world_show is not None:
                ...
                for mid in ids_for_kabsch:
                    if mid in pixel_centers:
                        ...
                        reproj_err[mid] = err
                        col_line = (0, 255, 0) if err < MAX_REPROJ_ERR_PX \
                                   else (0, 165, 255)
                        cv2.line(
                            img,
                            (int(px_exp), int(py_exp)),
        ...
        if T_cam_world_for_viz is not None:
            ...
            axis_img, _ = cv2.projectPoints(
                axis_pts, rvec_w, t_cw, camera_matrix, dist_coeffs
            )
            labels = ["X", "Y", "Z"]
            colors = [(0, 0, 255), (0, 255, 0), (255, 0, 0)]
            for lbl, p, col in zip(labels, axis_img.reshape(-1, 2), colors):
                cv2.putText(
                    img, lbl,
                    (int(p[0]) + 6, int(p[1]) - 6),
                    cv2.FONT_HERSHEY_SIMPLEX, S_MED,
                    col, T_BOLD, cv2.LINE_AA
                )
            ...
            ws_img, _ = cv2.projectPoints(
                ws_world, rvec_w, t_cw, camera_matrix, dist_coeffs
            )
            ws_pts = ws_img.reshape(-1, 2).astype(np.int32)
            cv2.polylines(
                img, [ws_pts], isClosed=True,
        ...
        # Afstand tussen twee laatste kliks
        ...
        if cam_pos_world is not None:
            ...
            n_avg_min = min(
                (len(marker_history[mid]) for mid in ids_for_kabsch),
                default=0
            )
            ...
            cv2.putText(
                img, "[LOCKED] (geen live markers gevonden)",
                (10, y0), cv2.FONT_HERSHEY_SIMPLEX,
