...
v2 change:
  * _build_dynamic_pose() now supports yaw_compensate / yaw_ref / yaw_sign,
    mirroring what _build_drop_pose_step() already   does. This lets the
    suction cup (or parallel jaws) rotate to match the part angle that
    SMA3 sends in pick:X,Y,Z,yaw,gripper.

    Because the TCP (snoeks_suction_tcp) is registered on the controller,
    rotating index 3 of pose_array ("A" / first Z in ZYZ) swings the
    flange around the TCP tip — the cup stays at (x+dx, y+dy, z+dz).
"""
...
_last_pick:       Optional[dict] = None
_last_drop_pose:  Optional[str]  = None
...
# Per-tool default flange orientation (ZYZ deg) when tool points "down".
DEFAULT_ORIENT_BY_TOOL = {
    "snoeks1":         (171.35, -137.40, -3.50),   # suction
    "snoeks_paralell": (  90,  -90,  -180),   # parallel
}
_active_tool: str = "snoeks1"   # updated on set_tcp
# ════════════════════════════════════════════════════════════════════════
#  TCP plumbing
# ════════════════════════════════════════════════════════════════════════
...
async def execute_pose(pose: dict,
    ...
    if "max_force" in pose:
        ...
        spd = pose["max_speed"]
        acc = pose.get("max_acc", spd)
        command_queue.put((move_type, pose_array, (spd, acc)))
    else:
...
def _build_dynamic_pose(step: dict) -> dict:
    ...
    Supports:
      dx, dy, dz                  : offsets in mm from the pick point
      rx, ry, rz                  : explicit override of flange orientation
      yaw_compensate (bool)       : rotate around base-Z by (pick.yaw - yaw_ref)
                                    so the suction cup / parallel jaws align
                                    with the part. Because the TCP is
                                    registered on the controller, the suction
                                    tip stays at (x+dx, y+dy, z+dz); only the
                                    flange swings around it.
      yaw_ref (deg)               : the yaw at which DEFAULT_RX is "correct".
                                    Default 0.0 — i.e. pick.yaw is taken as
                                    the absolute desired part angle.
      yaw_sign (+1 / -1)          : flip sign if the camera yaw rotates the
                                    opposite way from the robot's A axis.
      max_force                   : if present, descent is force-controlled.
      move_type, max_speed        : optional overrides.
    """
    ...
    dflt_rx, dflt_ry, dflt_rz = DEFAULT_ORIENT_BY_TOOL.get(
        _active_tool, (DEFAULT_RX, DEFAULT_RY, DEFAULT_RZ))
    rx = float(step.get("rx", dflt_rx))
    ry = float(step.get("ry", dflt_ry))
    rz = float(step.get("rz", dflt_rz))

    # ── Yaw compensation around tool-Z (= base-Z when tool points down) ──
    # In this ZYZ convention, index 3 ("A" / first Z) is the angle that
    # rotates the end-effector about the vertical axis without moving the
    # registered TCP tip. This matches drop_pose's yaw_compensate logic.
    if step.get("yaw_compensate", False):
        if "yaw" not in target:
            logger.warning("dynamic_pose: yaw_compensate requested but "
                           "pick has no 'yaw' — skipping rotation")
        else:
            yaw_ref = float(step.get("yaw_ref", 0.0))
            sign    = float(step.get("yaw_sign", 1.0))
            delta   = (float(target["yaw"]) - yaw_ref) * sign
            ...
            logger.info(f"  dynamic_pose yaw_compensate: "
                        f"pick.yaw={target['yaw']:.2f}  "
                        f"yaw_ref={yaw_ref:.2f}  sign={sign:+.0f}  "
                        f"Δ={delta:+.2f}  A {old_a:.2f} → {new_a:.2f}")
...
def _build_drop_pose_step(step: dict, poses: dict) -> dict:
    ...
    if step.get("yaw_compensate", False):
        if _last_pick is None or "yaw" not in _last_pick:
            logger.warning("drop_pose: yaw_compensate requested but no pick yaw")
        else:
            yaw_ref = float(step.get("yaw_ref", 35.0))
            delta   = float(_last_pick["yaw"]) - yaw_ref
            sign = float(step.get("yaw_sign", 1.0))
            delta *= sign

            idx = 3   # ← "A" is the tool-Z rotation in this configuration
            old_a = float(pose["pose_array"][idx])
            new_a = old_a + delta
            new_a = ((new_a + 180.0) % 360.0) - 180.0
            pose["pose_array"][idx] = new_a

            logger.info(f"  drop_pose yaw_compensate: "
                        f"pick.yaw={_last_pick['yaw']:.2f}  "
                        f"yaw_ref={yaw_ref:.2f}  Δ={delta:+.2f}  "
                        f"A {old_a:.2f} → {new_a:.2f}")

    pose["name"] = (f"{_last_drop_pose}"
                    f"{('+yaw' if step.get('yaw_compensate') else '')}"
                    f"{(f'{dz:+.0f}' if dz else '')}")
    ...
    for step in steps:
        ...
        if step_type == "sequence":
            ...
            await _gripper_action(step, close=False,
                                  command_queue=command_queue,
                                  result_queue=result_queue)
        elif step_type == "send_event":            # ← NEW
            name = step.get("name", "event")       # ← NEW
            msg  = f"event:{name}"                 # ← NEW
            logger.info(f"  → TCP event: {msg}")   # ← NEW
            tcp_send(msg)                          
        else:
...
async def main() -> None:
    ...
    while True:
        ...
        if msg.startswith("run:"):
            ...
            # ── After a successful sequence (e.g. pick & place),
            #    park the robot at A1_high before reporting done. ──
            if "A1_high" in poses:
                logger.info("Moving to A1_high after sequence…")
                try:
                    await execute_pose(poses["A1_high"],
                                       command_queue, result_queue)
                except Exception as e:
                    logger.error(f"Move to A1_high failed: {e}")
                    tcp_send(f"error:post_sequence_a1_high:{e}")
                    continue
            else:
                logger.warning("Pose 'A1_high' not found in JSON — skipping")
        ...
        if msg.startswith("set_tcp:"):
            name = msg.split(":", 1)[1].strip()
            command_queue.put(("set_tool", name))
            try:
                await wait_for_motion_complete(result_queue)
                _active_tool = name          # ← remember it
                tcp_send(f"done:set_tcp:{name}")
            except RuntimeError as e:
                tcp_send(f"error:set_tcp:{name}:{e}")
            continue
