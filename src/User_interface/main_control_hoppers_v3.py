...
- Er zijn nu 4 box-sensoren (S0..S3) op de Arduino. De rij-overgangen
  worden weer op SENSOR gedaan i.p.v. op tijd:
      row 4 -> wacht op rising edge S1 (na band START)
      row 3 -> wacht op rising edge S2
      row 2 -> wacht op rising edge S3
  De finale eject (na row 1) blijft op tijd (FINAL_EJECT_SECONDS).
- Na elke hopper-DONE wachten we POST_HOPPER_SETTLE_S seconden zodat
  de korrels uit de chute kunnen vallen voordat de doos doorschuift.

De Arduino streamt sensorregels in formaat "S<idx>:<0|1>" tussen
hopper-antwoorden door. Die regels worden hier gefilterd en gebruikt
voor de conveyor-logica; ze worden niet aangezien voor een hopper
DONE/SKIPPED/ERR antwoord.
"""
...
# Korte settle-tijd ná elke hopper DONE: laat de laatste korrels
# uit de chute vallen voordat de doos verder schuift / Y-as beweegt.
POST_HOPPER_SETTLE_S       = 5.0
# ----------------------------
...
def _ingest_line(line: str):
    """
    Sensorregels "S<idx>:<0|1>" -> sensor_state + rising edge flag,
    return None. Andere regels -> ongewijzigd terug.
    """
    if not line:
        return None
...
def settle_delay(seconds: float):
    """Zoals delay() maar print iets minder spammig en geen skip-prompt."""
    if seconds <= 0:
        return
    print(f".. settle {seconds:.2f}s (laat korrels vallen)")
    end = time.time() + seconds
    while time.time() < end:
        line = read_arduino_line()
        if line:
...
# Row -> bijbehorende sensor index op de Arduino:
#   row 4 = intake (S0)        - hoort bij main_control's advance
#   row 3 = wacht op S1
#   row 2 = wacht op S2
#   row 1 = wacht op S3
...
def conveyor_advance_to_row(conv: Conveyor, target_row: int):
    """
    Start de band en stop op de eerstvolgende rising edge van de
    sensor die hoort bij target_row.
    """
    sidx = ROW_SENSOR_IDX[target_row]
    print(f"[CONVEYOR] Doorschuiven naar row {target_row} "
          f"(wacht op S{sidx})")
    ...
    try:
        for i, s in enumerate(sequence):
            ...
            if result in ("skip", "timeout", "err"):
                ...
                # Laat korrels vallen voordat we doorschuiven of Y-as bewegen.
                settle_delay(POST_HOPPER_SETTLE_S)
