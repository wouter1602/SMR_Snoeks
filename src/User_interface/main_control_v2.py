...
Wijziging t.o.v. v4:
- Als er GEEN robotarm-jobs zijn (batch bestaat enkel uit hopper-
  deliveries), slaan we de SMA3-stap over MAAR gaan we wel door
  met advance_conveyor() en daarna de hopper-sequencer. In v4
  stopte het script in dat geval volledig, waardoor de doos nooit
  werd ingevoerd en de hopper-sequence niet startte.

Flow:
  1) Wacht WAIT_SECONDS.
  2) Lees active_batch.json, bouw robotarm-jobs.
  3) Als er robotarm-jobs zijn -> stuur naar SMA3 en wacht op 'done'.
     Geen jobs            -> SMA3-stap overslaan.
  4) advance_conveyor(): band AAN -> wacht op rising edge van box-
     sensor -> band UIT.
  5) Start hopper-sequencer (Main_control_hoppers.py) als subprocess.

Arduino-sketch publiceert per regel één karakter '0' (vrij) of '1'
(doos) van de enige fysieke box-sensor op pin D2.
"""
...
# ════════════════════════════════════════════════════════════════════════
HOPPER_SEQUENCER_SCRIPT  = BASE_DIR / "main_control_hoppers_v2.py"
...
# ════════════════════════════════════════════════════════════════════════
#  CONVEYOR — Modbus inverter + Arduino sensor (single sensor)
# ════════════════════════════════════════════════════════════════════════
class _SensorReader(threading.Thread):
    """
    Leest regels van de Arduino. De Arduino-sketch print per regel
    één karakter '0' of '1' voor de enige box-sensor. Info-regels
    (bv. "Gate OPEN ...") worden genegeerd voor sensorlogica.
    """
    def __init__(self, port: str, baud: int, timeout: float):
        ...
        try:
            ...
            pass
        self.state: int = 0
        self.rising_edges: "queue.Queue[float]" = queue.Queue()
...
def advance_conveyor() -> bool:
    """Band aan tot box-sensor een rising edge geeft, dan band uit."""
    print("\n[CONVEYOR] Start advance (wacht op box-sensor)...", flush=True)
    sensor  = None
    inv     = None
    running = False
    ...
    try:
        ...
        armed = (sensor.get_state() == 0)

        _with_retry(inv, inv.write_register, REG_CONTROL, CTRL_STF,
                    functioncode=6)
        running = True
        print(f"[CONVEYOR] Band loopt op {target_hz:.2f} Hz "
              f"(armed={armed}), wachten op doos...", flush=True)

        t_start = time.monotonic()
        while True:
            time.sleep(POLL_PERIOD_S)
            if sensor.get_state() == 0:
                armed = True
            if armed and sensor.got_rising_edge():
                try:
                    ...
                    f_out = float("nan")
                print(f"[CONVEYOR] Doos gedetecteerd "
...
# ════════════════════════════════════════════════════════════════════════
#  SMA3-stap (alleen als er robotarm-jobs zijn)
# ════════════════════════════════════════════════════════════════════════
def run_sma3_sequence(jobs: list) -> int:
    """
    Stuurt jobs naar SMA3 en wacht op 'done' of 'error'.
    Returncodes:
       0 = klaar
       1 = connect/verbinding mislukt
       2 = SMA3 meldde fout
    """
...
def main() -> int:
    ...
    has_hoppers = has_hopper_delivery(active_batch)

    print(f"[MAIN_CONTROL] {len(jobs)} robotarm job(s) "
          f"uit {total_deliveries} delivery(s). "
          f"Hopper-deliveries: {'ja' if has_hoppers else 'nee'}.",
          flush=True)
    ...
    # ── 1) SMA3-stap (alleen als er robotarm-jobs zijn) ─────────────────
    if jobs:
        rc = run_sma3_sequence(jobs)
        if rc != 0:
            return rc
    else:
        print("[MAIN_CONTROL] Geen robotarm-taken -> SMA3-stap "
              "overgeslagen.", flush=True)

    # Als er ook geen hopper-deliveries zijn, valt er verder niets te doen.
    if not has_hoppers:
        print("[MAIN_CONTROL] Geen hopper-deliveries -> klaar.",
              flush=True)
        return 0

    # ── 2) Conveyor advance: wacht op box-sensor ────────────────────────
