...
bool parseShortCommand(String command, String &payload) {
  int firstDash = command.indexOf('-');
  if (firstDash <= 0 || firstDash >= command.length() - 1) {
    return false;
  }

  int targetArduino = command.substring(0, firstDash).toInt();
  if (targetArduino != ARDUINO_ID) {
    return false;
  }

  payload = command.substring(firstDash + 1);
  payload.trim();
...
void handleCommand(String command) {
  ...
  String payload = "";
  if (!parseShortCommand(command, payload)) {
    String expectedPrefix = "arduino " + String(ARDUINO_ID) + " ";
    if (!command.startsWith(expectedPrefix)) {
      return;
    }

    payload = command.substring(expectedPrefix.length());
    payload.trim();
  }
