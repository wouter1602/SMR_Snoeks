...
IMPROVEMENTS in this version
  1) SAM3 only sees the workspace polygon — everything outside is blacked
     out before the image is handed to SAM3.
  2) Pipelined detection: while the Doosan is carrying the current part to
     the drop pose, SAM3 already starts looking for the next pick. Triggered
     by the 'event:lifted' message that the Doosan sends after the post-
     grasp +80 mm lift.
────────────────────────────────────────────────────────────────────────────
"""
...
# How long we are willing to wait for 'event:lifted' before falling back
# to non-pipelined behaviour (i.e. detect *after* the place finishes).
...
SHOW_GRID   = False
USE_CORNERS = False
# Shared with background sequence thread
latest_annotated_bgr   = None          # last SAM3 annotated frame (BGR)
...
# ─── NEW: workspace pixel mask ───────────────────────────────
def build_workspace_pixel_mask(shape_hw, pad_px=0):
    """
    Build a uint8 mask (0/255) of the workspace polygon projected
    into the camera image. Returns None if no calibration yet.
    """
...
def square_angle_px(bool_mask):
    """
    Angle (in *pixel* frame, degrees) of a square-ish blob.

    Uses cv2.minAreaRect on the largest contour of the mask and wraps the
    result into [-45, +45] degrees, which is the canonical range for a
    shape with 4-fold rotational symmetry (a square looks identical after
    any 90° rotation, so only the modulo-90 part is meaningful).

    Returns
    -------
    (angle_deg, (cx, cy), (w_px, h_px), squareness)
        angle_deg   : float in [-45, +45]
        (cx, cy)    : centre of the fitted rectangle in pixel coords
        (w_px,h_px) : side lengths of the fitted rectangle (px)
        squareness  : min(w,h)/max(w,h)  ∈ (0, 1];  1.0 == perfect square

    If the mask is empty / too small, returns (0.0, None, (0,0), 0.0).
    """
    import cv2
    import numpy as np
    ...
    if len(cnt) < 5:
        # Fall back to centroid + zero angle for tiny blobs
        ys, xs = np.where(bool_mask)
        return 0.0, (float(xs.mean()), float(ys.mean())), (0.0, 0.0), 0.0

    (cx, cy), (w, h), ang = cv2.minAreaRect(cnt)

    # cv2.minAreaRect returns angle in (-90, 0]. Convention: the angle is
    # measured from the +x axis to the first edge of the rectangle.
    # For square symmetry, fold into [-45, +45].
    a = ang
    # Ensure we describe orientation of the "long" edge consistently first
    if w < h:
        a += 90.0
    # Wrap to (-90, 90]
    a = ((a + 90.0) % 180.0) - 90.0
    # Square has 4-fold symmetry → collapse to (-45, 45]
    a = ((a + 45.0) % 90.0) - 45.0
...
def compute_pixel_angle(part, bool_mask):
    """
    Dispatcher: returns the pick angle in the *pixel* frame (degrees),
    choosing the method based on part['angle_mode'].

    - 'square'    -> square_angle_px  (good for caps, stickers, tiles, ...)
    - 'principal' -> principal_axes   (default, good for elongated parts)
    """
...
def detect_part(part_key, bgr_frame, depth_image, save_annot=True, show=True):
    ...
    # ── IMPROVEMENT #1: mask everything OUTSIDE the workspace so
    #    SAM3 only sees the tray area. Anything outside is blacked out.
    ...
    if not accepted_idx:
        ...
        yaw_info_deg = None  # informational only (suction)
        ...
        if part["gripper"] == "parallel":
            ...
            if part.get("compute_angle", False):
                ...
                if part.get("angle_mode", "principal") == "square":
                    # 4-fold symmetry → robot yaw only meaningful modulo 90°
                    info_yaw = ((info_yaw + 45.0) % 90.0) - 45.0
                else:
                    # 2-fold symmetry (elongated) → modulo 180°
    ...
    if save_annot:
        ...
        # Hand the annotated frame to the main GUI thread.
        # NEVER call cv2.imshow here — this function may run in a worker thread.
...
class DummyRobot:
    ...
    # ─── pipelined-detection stubs ─────────────────────────
    def clear_lifted(self): pass

    def wait_lifted(self, timeout=30.0):
        # In dummy mode just pretend the lift happens immediately
        time.sleep(0.1)
        return True
...
class DoosanRobot:
    ...
    def __init__(self, host, port):
        ...
        self._lifted_event = threading.Event()    # NEW
    ...
    def _on_message(self, msg: str) -> None:
        ...
        with self._lock:
            if msg == "doosan_ready":
                ...
                # Robot has finished the post-grasp lift — safe to capture
                # the next frame, the gripper is above the workspace.
...
# ============================================================
# 20) SEQUENCE RUNNER  (pipelined, Doosan-aware, named drop pose)
# ============================================================
# Background executor for SAM3 detection (one job at a time is enough,
# we only ever pre-fetch the NEXT pick).
_DETECT_EXECUTOR = ThreadPoolExecutor(max_workers=1)


def _detect_job(part_key):
    """Capture a fresh frame + run SAM3. Returns (best, color, depth)."""
...
def run_sequence(path=SEQUENCE_FILE):
    ...
    for step_idx, item in enumerate(items, 1):
        ...
        # Pipelined picking
        # ──────────────────────────────────────────────────────────────
        prefetched_future = None   # background detection for NEXT pick

        for pick_idx in range(1, count + 1):
            print(f"\n   ── PICK {pick_idx}/{count}  "
                  f"(tray {tray}, part {part_key}) ──")

            # 1) Get a detection — either prefetched or fresh
            best = None
            ...
            # Retry loop if no usable detection
            attempt = 0
            ...
            # 2) Build the robot pick coordinates
            ...
            # 3) Send pick + drop_pose + run
            ROBOT.clear_lifted()              # reset before starting
            ROBOT.send_pick(xr, yr, zr, robot_yaw, best["gripper"])
            ROBOT.send_drop_pose(drop_pose)

            # Kick off the motion sequence
            with ROBOT._lock if hasattr(ROBOT, "_lock") else threading.Lock():
                pass  # nothing to do, just being explicit
            ROBOT._done_event.clear() if hasattr(ROBOT, "_done_event") else None
            ROBOT._send(f"run:{doosan_seq}") if hasattr(ROBOT, "_send") \
                else ROBOT.run_named_sequence(doosan_seq,
                                              timeout=DOOSAN_SEQUENCE_TIMEOUT)

            # 4) PIPELINE: while the robot carries+places, ask SAM3 for
            #    the NEXT pick (only if there will be one).
            ...
            # 5) Now block until the place is done
            if ROBOT.name == "doosan":
                ...
                # Dummy backend already ran the "sequence" inline above.
